...
/**
 * FIDO passkey mark (fidoalliance.org "passkey icon"), inlined as a
 * currentColor SVG component so the one official symbol adapts to any text
 * color context (primary button, hero circle, empty state) and to dark mode —
 * which the static black/white SVG assets could not.
 *
 * Decorative by default (aria-hidden): every current usage sits next to an
 * explicit text label.
 */
export default function PasskeyIcon(props: SVGProps<SVGSVGElement>) {
  return (
    <svg viewBox="0 0 216 216" fill="currentColor" aria-hidden="true" {...props}>
      <path
        fillRule="evenodd"
        clipRule="evenodd"
