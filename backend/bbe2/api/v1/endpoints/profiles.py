...
from bbe2.utils.scopes import ProfilesScopes, GroupScopes
...
async def update_profile(
    ...
    groups = profile_crud.db_session.query(models.Group).filter(models.Group.id.in_(profile.group_ids)).all()
...
async def create_profile(
    ...
    groups = session.query(models.Group).filter(models.Group.id.in_(profile.group_ids)).all()
...
async def update_group(
    ...
    token: dict[str, Any] = Security(get_current_user, scopes=[str(GroupScopes.UPDATE)]),
    session: Session = Depends(get_db),
):
