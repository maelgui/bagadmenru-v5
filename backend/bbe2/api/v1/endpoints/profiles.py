...
async def get_my_profile(
    ...
    if not db_profile:
        db_profile = profile_crud.create(id=token["sub"])
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Profile not found")
    return db_profile

@router.put("/me", response_model=schemas.Profile)
async def update_my_profile(
    profile: schemas.ProfileUpdate, profile_crud: CRUDProfile = Depends(),
    token: dict[str, Any] = Security(get_current_user, scopes=[]),
):
    db_profile = profile_crud.find_one_by(models.Profile.id == token["sub"])
    if not db_profile:
...
async def get_profile(
    profile_id: str, profile_crud: CRUDProfile = Depends(),
...
async def list_profiles(
    profile_crud: CRUDProfile = Depends(),
    token: dict[str, Any] = Security(get_current_user, scopes=[ProfilesScopes.VIEW.value]),
):
